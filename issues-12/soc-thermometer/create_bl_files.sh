...
if [[ -z ${PATH_GCCARM} ]]; then
  OBJCOPY="/opt/simplicitystudio/developer/toolchains/gnu_arm/7.2_2017q4/bin/arm-none-eabi-objcopy"
...
if [[ -z ${PATH_SCMD} ]]; then
  COMMANDER="/opt/simplicitystudio/developer/adapter_packs/commander/commander"
...
if [[ -f $BOOTLOADER_FILE ]]; then
  ...
  "${COMMANDER}" gbl create "${PATH_GBL}/${OTA_APPLO_NAME}-bootloader.gbl" --app "${PATH_GBL}/${OTA_APPLO_NAME}.srec" --encrypt ${BOOTLOADER_FILE}
else
...
if [[ -f $GBL_ENCRYPT_KEY_FILE ]]; then
  ...
  if [[ -f $BOOTLOADER_FILE ]]; then
    echo "Bootloader file was found"
    "${COMMANDER}" gbl create "${PATH_GBL}/${OTA_APPLO_NAME}-bootloader-encrypted.gbl" --app "${PATH_GBL}/${OTA_APPLO_NAME}.srec" --encrypt ${GBL_ENCRYPT_KEY_FILE} --encrypt ${BOOTLOADER_FILE}
  else
...
if [[ -f $GBL_SIGING_KEY_FILE ]]; then
  ...
  if [[ -f "${PATH_GBL}/${OTA_APPLO_NAME}-signed.srec" ]]; then
    if [[ -f $BOOTLOADER_FILE ]]; then
      echo "Bootloader file was found"
      "${COMMANDER}" gbl create "${PATH_GBL}/${OTA_APPLO_NAME}-bootloader-signed.gbl" --app "${PATH_GBL}/${OTA_APPLO_NAME}-signed.srec" --sign ${GBL_SIGING_KEY_FILE} --encrypt ${BOOTLOADER_FILE}
    else
  ...
  if [[ -f $GBL_ENCRYPT_KEY_FILE ]]; then
    ...
    if [[ -f "${PATH_GBL}/${OTA_APPLO_NAME}-signed.srec" ]]; then
      if [[ -f $BOOTLOADER_FILE ]]; then
        echo "Bootloader file was found"
        "${COMMANDER}" gbl create "${PATH_GBL}/${OTA_APPLO_NAME}-bootloader-signed-encrypted.gbl" --app "${PATH_GBL}/${OTA_APPLO_NAME}-signed.srec" --encrypt ${GBL_ENCRYPT_KEY_FILE} --sign ${GBL_SIGING_KEY_FILE} --encrypt ${BOOTLOADER_FILE}
      else
  ...
  if [[ -f "${PATH_GBL}/${OTA_APPLO_NAME}-crc.srec" ]]; then
    if [[ -f $BOOTLOADER_FILE ]]; then
      echo "Bootloader file was found"
      "${COMMANDER}" gbl create "${PATH_GBL}/${OTA_APPLO_NAME}-bootloader-crc.gbl" --app "${PATH_GBL}/${OTA_APPLO_NAME}-crc.srec" --encrypt ${BOOTLOADER_FILE}
    else
  ...
  if [[ -f $GBL_ENCRYPT_KEY_FILE ]]; then
    ...
    if [[ -f "${PATH_GBL}/${OTA_APPLO_NAME}-crc.srec" ]]; then
      if [[ -f $BOOTLOADER_FILE ]]; then
        echo "Bootloader file was found"
        "${COMMANDER}" gbl create "${PATH_GBL}/${OTA_APPLO_NAME}-bootloader-crc-encrypted.gbl" --app "${PATH_GBL}/${OTA_APPLO_NAME}-crc.srec" --encrypt ${GBL_ENCRYPT_KEY_FILE} --encrypt ${BOOTLOADER_FILE}
      else
